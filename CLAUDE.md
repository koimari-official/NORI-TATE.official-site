...
### 問題9：お問い合わせフォームが送信先未設定で、実際には何も送信されていなかった（2026-07-09発見・対応中）
...
- **対応方針：** こいまりサイトと同様にFormspree等の外部フォーム送信サービスの導入を推奨。アカウント登録はオーナー側の対応が必要なため、Jobsからは実装の準備までで止めている
- **次回確認事項：** Formspree（または同等サービス）のエンドポイントが用意でき次第、送信処理を実装する
- **2026-07-09追記：** オーナーよりデータ蓄積の仕組み（こいまり同様のFirebase等）を希望する旨の指示があり、詳細確認中
