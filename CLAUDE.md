...
- **2026-07-09追記：** こいまり側と同じFormspreeフォーム（`https://formspree.io/f/mwvddbqk`）を共用し、送信時に`noritate.official@gmail.com`へメール通知が届くようにした（完全解決）
...
- **注意：** 住所は登記上「〒536-0007 大阪府大阪市城東区成育2丁目13-15 アイビーマンション103号室」（2026-09-08オーナー確認済み）。旧住所「大阪府大阪市北区梅田一丁目２番２号大阪駅前第2ビル１２－１２」は本店移転前の記載で、`privacy.html`に残っていたものを2026-09-08に新住所へ修正済み
- **残課題：** Formspree等を実際に導入する際は、4条の外国第三者提供条項に具体的な国名（米国等）を追記する必要がある

---

## 管理ページ（admin.html、2026-07-09新規作成、2026-09-07に「サイト内容」タブ追加）

お問い合わせ内容の蓄積・確認、および背景画像・文言の編集ができる管理画面。タブは「お問い合わせ」「サイト内容」の2つ。

- **ログイン**：Firebase Authentication（メール＋パスワード）。こいまり管理画面とは別のユーザー（`yoshida.koki1991@gmail.com` / パスワードはオーナー管理）を使用し、ログイン情報は完全に独立している
- **データ保存先**：こいまりと同じFirebaseプロジェクト（`koimari-tasting`）を使用（新規プロジェクト作成はしていない）。お問い合わせは`noritate/contacts`、サイト内容は`noritate/content`ノード
- **お問い合わせタブ**：一覧表示、ステータス切替（未対応／対応完了）、CSV出力、ゴミ箱（全削除は`trash/noritate_contacts`に退避してから削除。復元・完全削除が可能）
- **サイト内容タブ（2026-09-07追加）**：オーナーから「背景画像の設定や文言の修正をコードを触らずに自分でできるようにしたい」との要望を受けて新設。編集可能な項目：
  - 7枚の背景画像（Top/Concept/Name/Business/Key Principles・Join共用/Company/Contact）。アップロード時に自動で幅1920px・JPEG品質0.82に縮小圧縮してからBase64でRealtime DBに保存（こいまり管理画面のfileToDataUrlと同じ方式。Firebase Storageは使っていない）
  - 各セクションの見出し・本文・会社概要の各項目（会社名・設立・代表者・所在地・事業内容等）
  - **意図的に対象外**：電話番号・メールアドレス（会社概要表内の表示だけでなく、フッターのtel:/mailto:リンクや右サイドバー等、複数箇所に同じ値がハードコードされているため、CMS化すると表示とリンク先が食い違うリスクがある。変更したい場合はコード側を直接修正すること）
  - `index.html`側は`noritate/content`をonValueでリアルタイム購読し、値があれば上書き、無ければ従来のハードコード内容をそのまま表示する（フォールバック設計。何も保存しなければ現状維持）
  - セクションごとに独立した「保存する」ボタン（部分保存可）。末尾に「サイト内容を初期状態に戻す」ボタン（`noritate/content`ノードを削除するだけで、コード側のデフォルト文言・画像に戻る）
  - Firebaseルール：`koimari-site`リポジトリの`database.rules.json`に`"noritate": {".read": true, ".write": "auth != null"}`を追加してデプロイ済み（2026-09-07）。こいまりと共用DBのため、ルール変更は`claude code koimari site(ex)`側のリポジトリで行う
